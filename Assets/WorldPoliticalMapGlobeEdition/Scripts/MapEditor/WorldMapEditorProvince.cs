    ...
    public partial class WorldMapEditor : MonoBehaviour {
        ...
        bool GetProvinceIndexByGUISelection() {
            if (GUIProvinceIndex < 0 || GUIProvinceIndex >= provinceNames.Length)
                return false;
            if (GetEntityNameAndIndex(provinceNames[GUIProvinceIndex], out string name, out int index)) {
                GUIProvinceName = name;
                provinceIndex = index;
        ...
        int GetProvinceTransferToIndex() {
            ...
            if (GUIProvinceTransferToCountryIndex < 0 || _provinceNeighbourCountriesNames == null)
                return -1;
            if (GetEntityNameAndIndex(_provinceNeighbourCountriesNames[GUIProvinceTransferToCountryIndex], out _, out int index)) {
                targetProvinceIndex = index;
            }
        ...
        int GetProvinceMergeWithIndex() {
            ...
            if (GUIProvinceMergeWithIndex < 0 || _provinceNames == null)
                return -1;
            if (GetEntityNameAndIndex(_provinceNames[GUIProvinceMergeWithIndex], out _, out int index)) {
                targetProvinceIndex = index;
            }
            return targetProvinceIndex;
        }
