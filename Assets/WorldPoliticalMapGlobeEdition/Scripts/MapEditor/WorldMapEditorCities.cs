...
namespace WPM {
    public partial class WorldMapEditor : MonoBehaviour {
        ...
        int lastCityCount = -1;
        string[] _cityNames;
        int lastCityCountryIndex = -1;

        public string[] cityNames {
            get {
                if (map.cities != null && lastCityCount != map.cities.Count || lastCityCountryIndex != countryIndex) {
                    cityIndex = -1;
                    ReloadCityNames();
                }
                return _cityNames;
        ...
        void UpdateCityVisualPosition() {
            Renderer cityRenderer = map.cities[cityIndex].renderer;
            if (cityRenderer != null) {
                cityRenderer.transform.localPosition = map.cities[cityIndex].localPosition * 1.001f;
            }
        }
        ...
        bool GetCityIndexByGUISelection() {
            if (GUICityIndex < 0 || GUICityIndex >= cityNames.Length)
                return false;
            if (GetEntityNameAndIndex(cityNames[GUICityIndex], out string name, out int index)) {
                GUICityName = name;
                cityIndex = index;
                return true;
            }
        ...
        public void ReloadCityNames() {
            ...
            lastCityCountryIndex = countryIndex;
        ...
        void SyncGUICitySelection() {
            ...
            if (!string.IsNullOrEmpty(GUICityName)) {
                string guiName = GUICityName;
                if (cityIndex >= 0) {
                    guiName += " (" + cityIndex + ")";
                    // look for exact gui city name
                    for (int k = 0; k < _cityNames.Length; k++) {
                        if (_cityNames[k].Equals(guiName)) {
                            GUICityIndex = k;
                            return;
                        }
                    }
                ...
                // otherwise traverse the list of new citynames
